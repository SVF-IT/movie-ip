...
export default function NewProductionHousePage() {
  const router = useRouter();
  const { allowed, loading: permLoading } = useRequirePermission("create", "production_house", "/production-houses");
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await createProductionHouse({ name: name.trim() });
      router.push("/production-houses");
    } catch (err) {
      toast.error(err instanceof Error ? err.message : "Failed to create production house");
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="relative overflow-hidden rounded-xl bg-slate-900/60 border border-slate-800/60 backdrop-blur-xl p-6 shadow-2xl">
        <div className="absolute top-0 left-0 w-full h-0.5 bg-gradient-to-r from-cyan-600 via-orange-500 to-transparent" />
        ...
        <div className="relative flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">Add a new production house or studio</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex justify-end gap-3">
        ...
        >
          {saving ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Creating…</> : "Create Production House"}
        </Button>
      </div>
    </div>
  );

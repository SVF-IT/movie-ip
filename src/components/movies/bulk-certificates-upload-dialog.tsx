...
import { createCensorCertificate, CENSOR_CERTIFICATE_ACCEPTED_TYPES } from "@/lib/api/censor-certificates";
import { useAuth } from "@/contexts/auth-context";

interface BulkCertificatesUploadDialogProps {
    open: boolean;
...
interface MatchResult {
    ...
    status: "matched" | "no-match" | "uploading" | "success" | "error";
    error?: string;
    selected: boolean;
}
...
export function BulkCertificatesUploadDialog({
    open,
    onOpenChange,
    onSuccess,
}: BulkCertificatesUploadDialogProps) {
        ...
        try {
            ...
            const newMatches: MatchResult[] = files.map((file) => {
                ...
                const matched = (movies || []).find((m: { id: string; title: string }) => {
                    ...
                    const tightTitle = tightNormalize(m.title);
                    return normTitle === normalizedFileName || tightTitle === tightFileName;
                });
            ...
            console.error("Error matching certificates:", err);
    ...
    const handleUpload = async () => {
        ...
        for (let i = 0; i < toUpload.length; i++) {
            ...
            if (!ACCEPTED_MIME_TYPES.includes(match.file.type)) {
            ...
            try {
                await createCensorCertificate({
                    movieId: match.matchedMovieId!,
                    file: match.file,
                    tag: null,
        ...
        <Dialog open={open} onOpenChange={(val) => {
            ...
            <DialogContent className="max-w-4xl h-[90vh] flex flex-col p-0 overflow-hidden">
                <DialogHeader className="p-6 pb-2">
                    ...
                    <DialogDescription>
                        Select censor certificate PDFs to upload. We'll automatically match them to movies in your catalog by filename.
                    </DialogDescription>
                </DialogHeader>
                ...
                <div className="flex-1 overflow-hidden flex flex-col p-6 pt-2 min-h-0">
                    {matches.length === 0 ? (
                        <div className="flex-1 border-2 border-dashed border-muted-foreground/20 rounded-[12px] flex flex-col items-center justify-center p-12 bg-muted/5">
                            ...
                            <h3 className="text-lg font-semibold mb-2">Select Certificate Files</h3>
                            <p className="text-sm text-muted-foreground text-center mb-6 max-w-xs">
                                Drag and drop your certificate PDFs here, or click to browse. Filenames should ideally match movie titles.
                            </p>
                            <input
                                type="file"
                                multiple
                                accept={CENSOR_CERTIFICATE_ACCEPTED_TYPES.join(",")}
                                className="hidden"
                            ...
                            <div className="flex gap-4">
                                <Button asChild size="lg" className="h-11 px-8">
                                    <label htmlFor="certificate-upload" className="cursor-pointer flex items-center gap-2">
                                        <FileText className="h-4 w-4" />
                                        Select Files
                                    </label>
                                </Button>
                                <Button asChild variant="outline" size="lg" className="h-11 px-8">
                                    <label htmlFor="certificate-folder-upload" className="cursor-pointer flex items-center gap-2">
                            ...
                            {uploadSummary && (
                                <div className="border border-border/60 rounded-[12px] p-4 bg-muted/20 space-y-3 shrink-0 max-h-64 overflow-y-auto">
                                    <div className="flex items-center gap-2 text-sm font-semibold">
                                        ...
                                        Uploaded {uploadSummary.successCount} certificate{uploadSummary.successCount === 1 ? "" : "s"}
                                    </div>
                        ...
                        <>
                            ...
                            >
                                {isProcessing ? (
                                    ...
                                    `Upload & Link ${selectedCount} Certificates`
                                )}
                            </Button>
                        </>
                    )}

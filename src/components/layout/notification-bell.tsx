...
export function NotificationBell() {
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent
        className="w-80 p-0 bg-(--panel-solid)/95 backdrop-blur-xl border-(--svf-border) shadow-2xl rounded-[12px] overflow-hidden"
        align="end"
      >
        ...
        <div className="max-h-[320px] overflow-y-auto custom-scrollbar">
          {loading ? (
            <div className="p-8 text-center text-sm text-(--text-faint) animate-pulse">Loading notifications...</div>
          ) : notifications.length === 0 ? (
            <div className="p-8 text-center flex flex-col items-center justify-center space-y-2">
              <Bell className="h-8 w-8 text-slate-700/50 mb-2" />
              <p className="text-sm font-medium text-(--text-faint)">All caught up</p>
              <p className="text-xs text-(--text-faint)">No new notifications</p>
            </div>
          ) : (
            notifications.map((n) => {
                ...
                >
                  ...
                  <div className="flex-1 min-w-0 pr-6">
                    <div className="flex items-start gap-1.5 mb-1">
                      <Icon className="h-3.5 w-3.5 mt-0.5 shrink-0 text-(--text-faint)" />
                      <p className="text-sm font-medium leading-tight text-(--text)">{n.title}</p>
                    </div>
                    <p className="text-xs text-(--text-faint) mt-0.5 line-clamp-2 leading-relaxed opacity-90">{n.message}</p>
                    <p className="text-[10px] text-(--text-faint) mt-2 font-medium tracking-wide">
                  ...
                  <Button
                    variant="ghost"
                    size="sm"
                    className="h-7 w-7 p-0 shrink-0 absolute right-3 top-4 opacity-0 group-hover:opacity-100 transition-opacity text-(--text-faint) hover:text-green-400 hover:bg-green-400/10"

...
export function InternetDashboardTable({
  ...
  const filteredMovies = licensorFilter.trim()
        ...
        <Popover open={licensorOpen} onOpenChange={(o) => { setLicensorOpen(o); if (!o) setLicensorSearch('') }}>
          <PopoverTrigger asChild>
            <Button variant="outline" size="sm"
              className={`h-9 w-40 justify-start gap-1.5 text-xs font-normal bg-(--bg-raise) border-(--svf-border) hover:bg-(--hover) hover:border-(--svf-border-strong) transition-colors ${licensorFilter ? 'border-blue-500/60 text-blue-400 bg-blue-500/5' : 'text-(--text-faint)'}`}>
